...
def findAccessible(grid):
    """find all accessible rolls on the current grid, return (count, list of [i][j] coords of rolls that are accessible)"""
    accessible_count = 0
    accessible_coords = []
    for i in range(0, len(grid)):
        for j in range(0, len(grid[0])):
            if(grid[i][j] == '@'):
                ...
                print('.', end='')
        print("")
    return accessible_count, accessible_coords

def solveGrid(grid):
    """find the solution: how many rolls can be removed"""
    total_count = 0
    while(True):
        count, to_remove = findAccessible(grid)
        if count == 0:
            break
        total_count += count
